...
/**
 * Provides various functions for generate and formatting output.
 */
class OutputTools {
	 ...
	 * building the resulting table. If this array is a simple string array, it
	 * will be used to narrow and potentially re-order the result set according
	 * to the order and presence of a given column within this array. If this
	 * array is associative, the keys of the array will function similarly to
	 * what was just described, while the values of each key will be used as a
	 * replacement for the header for the column being described. 
	...
	function to_ascii_table(array $data, array $headers = null): string {
		...
		$data = array_map(
			callback: fn($row) =>
				is_object($row) ? get_object_vars($row) : $row,
			array: $data,
		);
		...
		$result .= join("", [
			"| ",
			join(" | ", array_map(
				fn($header_title, $width): string =>
					str_pad($header_title, $width),
	 ...
	 * what was just described, while the values of each key will be used as a
	 * replacement for the header for the column being described.
	 * @return string A string containing an HTML table.
	 */
